...
Check that Python 3.12 is active:
```bash
python --version
```

Install the package in editable mode. Make sure you're in the directory that contains the `setup.py` file, or replace `.` with the full path to the unzipped folder:
```bash
pip install -e .
# or, for example:
...
Run the automation scripts to compare locking modes and alleviation techniques:
```bash
...
The **Assumed Natural Strain (ANS)** method is fully implemented at the **element level**, making it straightforward to adapt to other codes and applicable to a wider range of problems. To the best of our knowledge, this method is not currently available as **open-source**, despite its classical status and widespread use in commercial software. Through this work, we hope to help filling this gap by presenting our **open-source implementation**. Also, to the best of our knowledge, there is no available **open-source finite-element code** that uses **convected curvilinear coordinates**, rather than the usual **Cartesian** ones, and the topic is rarely (if any) addressed in **standard textbooks**. We hope this implementation can be of help for both **learners** and **researchers** interested in this kind of **formulations**.
