...
## 📘 Nonlinear Finite Element Procedure (Summary of Chapter 3)

This section summarizes the main concepts and equations from *Chapter 3* of the reference text. It outlines the **nonlinear FEM procedure** based on the **total Lagrangian formulation**, suitable for large deformations and hyperelastic materials.

### 🔹 Weak Form

Derived from the **principle of virtual work**, not the energy functional:
- Suitable for **non-conservative** and **inelastic** problems.
...
- $g^{\mathrm{int}} := \int_{\Omega_0} \mathbf{S} : \delta \mathbf{E} \, \mathrm{d}V$
- $g^{\mathrm{ext}} := \int_{\partial \Omega_0} \mathbf{t}_0 \cdot \delta \mathbf{x} \, \mathrm{d}A$

### 🔹 Linearization

Nonlinear FEM requires iterative solvers (e.g., Newton-Raphson). We use **directional derivatives** for linearization.
...
- $\mathbb{C}(\mathbf{E})$: Lagrangian elasticity tensor (depends on material model).
- $\mathbf{S}$: second Piola-Kirchhoff stress tensor.

When loads are **conservative**, $\Delta g^{\mathrm{ext}} = 0$.

### ⚠️ Follower Loads (Non-Conservative)

If loads **depend on deformation**, such as **pressure normal to a deforming surface**, we must include:

- Nonzero $\Delta g^{\mathrm{ext}}$
- Extra **unsymmetric** tangent stiffness contribution
- Load must be **updated each iteration**

Example pressure load:

$$
\hat{p} = -\left(\frac{12M}{I}\right)\zeta, \quad \zeta \in \left[-\frac{h}{2}, \frac{h}{2}\right]
$$

### 🔹 Spatial Discretization

Using **isoparametric elements**, displacements and geometry are interpolated using the same shape functions.

Discretized internal virtual work:

$$
g_{\mathrm{int}} \approx \sum_e \mathbf{u}^T_e \mathbf{f}^{\mathrm{int}}_e
$$

Tangent stiffness matrix:

$$
\mathbf{K} = \sum_e (\mathbf{K}^{\mathrm{mat}}_e + \mathbf{K}^{\mathrm{geo}}_e - \mathbf{K}^{\mathrm{follower}}_e)
$$

### 🔹 Higher-Order Brick Elements

The implementation supports **arbitrary interpolation in the axial direction**, with 2 nodes in each cross-section direction (η, ζ), and `n_ξ ≥ 2` nodes in length.

Shape functions in 3D (tensor product form):

$$
N_{IJK}(\xi, \eta, \zeta) = N_I(\xi) N_J(\eta) N_K(\zeta)
$$

Derivatives are computed analytically using the **product rule** and standard **Lagrange polynomials**.

---

**📌 Notes:**
- The formulation is based on the **total Lagrangian approach**.
- All quantities (stress, strain, geometry) are measured w.r.t. the **reference configuration**.
- Tangent matrix can be **non-symmetric** if follower loads are present.

📂 Add visualizations for strain energy, follower load implementation, and isoparametric mesh using your figures.
