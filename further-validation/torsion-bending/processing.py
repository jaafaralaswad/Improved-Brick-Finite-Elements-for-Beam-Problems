...
def compute_external_force(E, length, height, width, n_load_steps, inc,
                            coords, connect, ne, ne_L, numel, ngp_c, ndof, u):
    """
    Compute the external force vector and load stiffness matrix
    matching MATLAB concentrated nodal force logic.
    ...
    ndof_e = 3 * ne
    F_ext = np.zeros((ndof, 1))
    ke_l = np.zeros((ndof_e, ndof_e))  # Unused in MATLAB logic, returned as zero

    # List of loaded elements — MATLAB: list_e_surf = numel
    list_e_surf = [numel - 1]  # Python is 0-indexed

    # Concentrated force (already P/4 in MATLAB)
    force = 300 / 4
    load_value = force * inc / n_load_steps

    # Loop over loaded elements (in practice, just the last one)
    for ie in list_e_surf:
        fe_ext = np.zeros((ndof_e, 1))

        # MATLAB:
        # fe_ext(N-1) = load_value
        # fe_ext(N-4) = load_value
        # fe_ext(N-7) = load_value
        # fe_ext(N-10) = load_value

        # These correspond to Y-direction DOFs of last 4 nodes in element
        fe_ext[-2]   = load_value  # node 4, y
        fe_ext[-5]   = load_value  # node 3, y
        fe_ext[-8]   = load_value  # node 2, y
        fe_ext[-11]  = load_value  # node 1, y
