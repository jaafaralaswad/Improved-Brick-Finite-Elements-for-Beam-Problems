...
cases = [
    {"label": "Baseline (numel=10, ANS off)", "numel": 10, "ne_L": 2, "ngp_l": 2, "ANS_membrane": False, "ANS_shear": False, "ANS_curvature": False},
    {"label": "h-refinement (numel=20, ANS off)", "numel": 20, "ne_L": 2, "ngp_l": 2, "ANS_membrane": False, "ANS_shear": False, "ANS_curvature": False},
    {"label": "p-refinement (ne_L=3, ANS off)", "numel": 10, "ne_L": 3, "ngp_l": 3, "ANS_membrane": False, "ANS_shear": False, "ANS_curvature": False},
    {"label": "ANS applied (ANS on)", "numel": 10, "ne_L": 2, "ngp_l": 2, "ANS_membrane": True, "ANS_shear": True, "ANS_curvature": True},
    {"label": "Reduced integration (ngp_l=ne_L-1)", "numel": 10, "ne_L": 2, "ngp_l": 1, "ANS_membrane": False, "ANS_shear": False, "ANS_curvature": False}
]
...
for case in cases:
    validate_inputs(width, height, length, E, nu, case["numel"], case["ne_L"], ngp_c, case["ngp_l"],
                    n_load_steps, max_iterations, tolerance)

    coords, nnode_W, nnode_H, nnode_L, ndof, connect, ne = create_mesh(width, height, length, case["numel"], case["ne_L"])
    ...
    u, displacements_all = newton_raphson_solver(
        ...
        case["numel"], ne, case["ne_L"], ngp_c, case["ngp_l"],
        n_load_steps, max_iterations, tolerance,
        case["ANS_membrane"], case["ANS_shear"], case["ANS_curvature"]
    )

    tip_displacements = [
        compute_tip_displacement(u_i, coords, connect, shape_functions_3D, case["ne_L"])
...
for i, (label, tips) in enumerate(tip_displacements_all):
    ...
    plt.plot(k_normalized, x_disp_normalized,
             color=colors[i], marker=markers[i],
             linestyle=linestyles[i], linewidth=2, label=label)
...
plt.plot(k_normalized, analytical, linestyle='-', color='black', linewidth=2, label="Analytical solution")

plt.xlabel(r"$k = \dfrac{ML}{2\pi EI}$", fontsize=12)
plt.ylabel(r"Normalized tip displacement $u_x / L$", fontsize=12)
plt.title("Comparison of Locking Alleviation Techniques", fontsize=14)
plt.grid(True)
plt.legend()
plt.tight_layout()

plt.savefig(figures_dir / "compare_configurations.png", dpi=300)
plt.close()
